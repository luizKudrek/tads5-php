...
class AppController extends Controller
{
    ...
    public function initialize(): void
    {
        ...
        //$GLOBALS['token'] = 'DOeQnGSnLhT6QwoICKl8CoCU1oP1TUTcCOQ5Zp6zzSKYnWuope';
    ...
    protected function buscaToken()
    {
        ...
        if (!empty($this->request->getHeader('Autenticacao')[0])) {
            ...
            $sql = 'SELECT * FROM autenticacaos WHERE autenticacao = :token AND expira >= :dtAtual LIMIT 1';
            $resultado = $GLOBALS['connection']->execute($sql,
                ['token' => $token, 'dtAtual' => date('yY-m-d')])
                ->fetch('assoc');
        }

        return$resultado;
    }
    protected function sendEmail($emailTo, $emailSubject, $emailDeliver){
        $mailer = new Mailer();
        $mailer->setEmailFormat('html')
        ->setFrom([env('EMAIL_TRANSPORT_USERNAME')=> 'TADS 5'])
            ->setTo(is_object($emailTo) ? $emailTo->email : $emailTo)
