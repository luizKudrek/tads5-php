...
class PagesController extends AppController
{
    ...
    public function logar()
    {
        ...
        if ($result && $result->isValid()) {
            ...
            $autenticacao['autenticacao'] = hash('sha256', $senha);
            $autenticacao['user_id'] = $result->getData()['id'];
            $autenticacao['expira'] = date('Y-m-d', strtotime('+4 days'));

            try {
                $existe = $this->Autenticacaos->find()->where(['user_id' => $autenticacao['user_id']])->first();

                if ($existe) {
                    $existe['autenticacao'] = $autenticacao['autenticacao'];
                    $existe['expira'] = $autenticacao['expira'];
                    $this->Autenticacaos->saveOrFail($existe);
                } else {
                    $this->Autenticacaos->saveOrFail($autenticacao);
                }

                $response['autenticacao'] = $autenticacao['autenticacao'];
